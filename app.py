...
def fetchWeatherData(city, units):
    ...
    return {
        ...
        "sunrise": datetime.fromtimestamp(weather_info["sys"]["sunrise"]),
        "sunset": datetime.fromtimestamp(weather_info["sys"]["sunset"]),
        "units_letter": get_letter_for_units(units),
    }
...
def comparison_results():
    ...
    city1_info = fetchWeatherData(city1, units)
    city2_info = fetchWeatherData(city2, units)
    ...
    temp_comparison = (
        "warmer" if city1_info["temp"] > city2_info["temp"] else "colder"
    )

    humidity_comparison = (
        "greater" if city1_info["humidity"] > city2_info["humidity"] else "less"
    )

    wind_comparison = (
        "greater" if city1_info["wind_speed"] > city2_info["wind_speed"] else "less"
    )

    sun_set_comparison = (
        "earlier" if city1_info["sunset"] < city2_info["sunset"] else "later"
    )
    ...
    context = {
        ...
        "date": datetime.now().strftime("%m - %d - %Y"),
        "units_letter": get_letter_for_units(units),
        "num_degrees": round(abs(city1_info["temp"] - city2_info["temp"]), 2),
        ...
        "wind_comparison": wind_comparison,
        "wind_speed_diff": round(
            abs(city1_info["wind_speed"] - city2_info["wind_speed"]), 2
        ),
        "sun_set_comparison": sun_set_comparison,
        "sun_set_diff": abs(
            int(city1_info["sunset"].strftime("%H"))
            - int(city2_info["sunset"].strftime("%H"))
        ),
    }
